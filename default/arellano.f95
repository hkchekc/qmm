            ...
            DO CHOICEIDX=1,NA ! LOOP OVER CHOICE OF ASSET PRIME
                ...
                IF (CONSUM > 0.) THEN
                    ...
                    ELSE  ! IF DEBT, THEN DECIDE REPAY OR NOT
                        ...
                        IF (EV_SHOCK) THEN
                            DO NZI=1, NZ
                                NU = NU + EMP_MARKOV(SCOLIDX,NZI)*(VFUNC_DEF(CHOICEIDX,NZI)+&
                                EULERGAMMA/ALPHA+LOG(1+EXP(VFUNC_CLEAN(ZERO_LOC,NZI)-VFUNC_DEF(CHOICEIDX,NZI)))/ALPHA)
                            ENDDO
                        ELSE
                            DO NZI=1, NZ
        ...
        DO SCOLIDX=1, NZ
            ...
            IF (EV_SHOCK) THEN
                DO NEXT_STATE_IDX=1, NZ
                    RISK = RISK + EXP(ALPHA*VFUNC_CLEAN(CHOICE, NZ))/(EXP(ALPHA*VFUNC_CLEAN(CHOICE, NZ))+&
                    EXP(ALPHA*VFUNC_DEF(ZERO_LOC, NZ)))
                ENDDO            
                QZERO = RISK/(1.+INTEREST)
            ELSE
                DO NEXT_STATE_IDX=1, NZ
