...
def _optimal_m(r, p):  # void function
    v0_spline = interpolate.CubicSpline(p.s_grid, r.vfunc0, bc_type="not-a-knot")
    def find_m(m, si, p, r):
    ...
    vals_exhaust = r.p_star*_final_net_prod(p.s_grid, _find_n(p.s_grid, p.theta_m, p.theta_n, p.eta, r.p_star), p.theta_m,
                             p.theta_n, p.s_grid, p.store_cost, r.wage) + p.beta*v0_spline(0)
    exhaust_mx = vals_exhaust > r.vals_s1_new
    r.vals_s1_new[exhaust_mx] = vals_exhaust[exhaust_mx]
